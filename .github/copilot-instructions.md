...
- E2E & on-device verification: **Argent** (`@swmansion/argent`, MCP server wired in
  `.mcp.json`). Maestro/Detox are banned — do not add them back.
  - While developing, verify UI changes yourself on the simulator/emulator with the
    Argent tools (tap, type, `describe`, screenshot) instead of assuming or asking —
    use the `argent-test-ui-flow` / `argent-device-interact` skills.
  - Regression E2E lives as recorded flows in `.argent/flows/*.yaml`. Author them by
    live recording via the `argent-qa-flows` skill (never hand-write flow YAML);
    the canonical smoke flow is `smoke`.
...
- Provider-agnostic AI layer: `src/services/ai/` (Anthropic / OpenAI / Gemini adapters + production proxy). One `AIProvider` interface, SSE streaming via `expo/fetch`.
- SECURITY: provider API keys never ship in release builds — `EXPO_PUBLIC_AI_PROVIDER=proxy` in production; direct provider mode is development-only. The proxy contract is documented in `src/services/ai/providers/proxy.ts`.
- Structured output: pass `schema` (JSON Schema, `additionalProperties: false`) on `AIRequest`.
- Always handle `stopReason === 'refusal'` before trusting response text.
- Chat example: `src/app/chat.tsx` + `useAIChat` — delete with the showcase if unused.

## Working in this repo (agents)

- One-shot check: `npm run verify` (typecheck + lint + tests). Run it before finishing.
- Scaffolding: `npm run gen screen <name>` / `npm run gen feature <name>` emit rule-conforming skeletons — prefer them over hand-writing boilerplate.
- Rule files for other assistants (`AGENTS.md`, `.cursor/rules/`, `.github/copilot-instructions.md`) are GENERATED from this file — edit CLAUDE.md, then `npm run sync:ai-rules`.
